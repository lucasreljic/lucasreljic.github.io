  ...
  return (
    <Container fluid>
      ...
      <Text>Python code that reads email sent to an email address that contains information on which
        stocks
        to track at certain
        prices. It then creates threads to track the stock prices and sends an email in return when the "Market Alarm"
        is triggered.</Text>
    </Container>
  );
}
...
const PiLaptop = () => {
  return (
    <Container fluid>
      ...
      <h2>2022</h2>
      <Slide>
        <SlideBox photo='photolaptop1'>
          Photo 1
        </SlideBox>
        <SlideBox photo='photolaptop2'>
          Photo 2
        </SlideBox>
      </Slide>
      <Text>
...
const Gokart = () => {
  return (
    <Container fluid>
      ...
      <h2>2018</h2>
      <Slide>
        <SlideBox photo='photokart2'>
          Front
        </SlideBox>
        <SlideBox photo='photokart1'>
          Back
        </SlideBox>
        <SlideBox photo='photokart3'>
          Driving the GoKart
        </SlideBox>
      </Slide>
      <Text>
        One of the largest individual projects I undertook. It required learning how to weld, and a significant amount
  ...
  return (
    <Container fluid>
      ...
      <Text>
        ...
        of the system. I decided to house the potentiometers and the temperature sensor in the same box. I did this
        because they are small and lightweight so it would be simpler to add them. I decided to have 4 “modules”
        that
        each have a transmitter that sends a signal to the pumps to turn them on and off. There are 4 modules
        because
        there are only 4 potentiometers anymore and I would not be able to fit them onto the design.
      </Text>
      <br></br>
      <Text>
        The final design enables the user to adjust exactly how much to water their plant every 10 days. The
        system
        automatically calculates how much to add or subtract water amount based on temperature and humidity. The
        criteria shaped it to be a small compact design that is easy to set up and use. The use of a Raspberry Pi
        makes
        it suitable to run for months on end. Multiple modules allow adding more plants easy, even while the system
        is
        still running. The connector design makes it difficult to install a module incorrectly. A smaller pump
        allows
        for more precise watering as well as the ability to place in almost any water tank. The cost to manufacture
        a
        single module and system is $125.87 making the device affordable. This price will decreases as more systems
        are
        manufactured as many of the parts needed can be bought in bulk. The Automatic plant watering system is
        perfect
  ...
  return (
    <Container fluid>
      ...
      <Slide>
        ...
        <SlideBox photo='photosanitizer2'>
          Side View
        </SlideBox>
        <SlideBox photo='photosanitizer3'>
      ...
      <Text>
        ...
        from a
        venture outside. While returning from these ventures you absolutely need to sanitize and wash your hands before
        you touch
        anything in your home to kill the pesky Coronavirus.
      </Text>
      <br></br>
      <Text>
        The Hand Sanitiser solves the issue of
        contaminating your home before you properly wash or
        sanitize them, as can often happen while handling a bottle of hand sanitizer or turning the tap while washing
        your hands. The Hand Sanitizer is designed so that the user does
        not have to touch any surfaces to sanitize
        their hands. It is to be secured on the front door so that you or your family members while sanitizing their
        hands before touching the handle. It works based on an ultrasonic sensor that detects when something is
        close by
        then an Arduino sets off a servo that sprays the bottle that contains isopropyl alcohol. The bottle sprays a
        fine mist so that the droplets are spread out and you can effectively kill the coronavirus. The design has
        proved to be lightweight and small. The sensors are in prime position for the spray bottle to
        spray a person’s hands. The easy refill method makes it simple to use. This along with the easy setup of
        only
        having to plug it into an outlet makes for an "easy to use product" for anyone. It also has no fancy cables
        and
        only requires a USB connection to a power adapter. The accurate ultrasonic sensor along with changes
        in the code allows for great detection and minimal waste of isopropyl alcohol. The max amount of “sprays”
        the
        code will allow for a single detection is 4 as that is more than enough to sanitize your hands thoroughly as
        found by testing. It also detects if a person left before waiting for 4 sprays and automatically will stop.
        The
        spray bottle volume is 60ml so that is more than 414 “sprays” or if you use the Hand Sanitizer twice a day
        that
        is 52 days before needing a quick refill by removing the bottle and filling it with isopropyl alcohol. Most
        automatic sanitizer/soap dispensers online are around $50-$150, but my design costs only $32.04 to
        manufacture.
      </Text>
    </Container>
  )
...
export { Gokart, Plant, Sanitizer, RCplanes, Project1a, PiLaptop, Marketalarm, Openmv };